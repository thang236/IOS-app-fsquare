...
class CheckOutViewController: UIViewController {
    ...
    init(viewModel: CartViewModel) {
        ...
        self.clientOrderCode = viewModel.generateRandomID()
    ...
    private func createOrder() {
        ...
        let order = Order(
            ...
            shippingFee: self.shippingFee ?? 0,
            content: "\(bagData.count) đôi giày",
            isFreeShip: false,
            ...
            .sink { orderResponse in
                if let orderResponse = orderResponse {
                    if orderResponse.status == HTTPStatus.success.message || orderResponse.status == HTTPStatus.created.message {
                        ...
                        self.showToast(message: "Đặt hàng thành công", chooseImageToast: .success)
                        self.navigationController?.popViewController(animated: false)
                        
                    } else {
                        self.showToast(message: "Đặt hàng thất bại", chooseImageToast: .error)
                    }
                }
            }.store(in: &viewModel.cancellables)
    ...
    private func createHeaderItem(for sectionType: CheckOutCollectionType) -> [NSCollectionLayoutBoundarySupplementaryItem] {

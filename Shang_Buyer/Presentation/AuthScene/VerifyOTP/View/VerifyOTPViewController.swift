...
class VerifyOTPViewController: UIViewController {
    ...
    func endCountdown() {
        ...
        refreshOTPButton.setTitle("Resend Code", for: .normal)
    ...
    private func setupBindings() {
        viewModel.$errorMessage
            ...
            .receive(on: DispatchQueue.main).sink { [weak self] errorMessage in
                self?.showToast(message: "\(errorMessage) OTP, please try again", chooseImageToast: .error)
    ...
    private func validateInput() {
        guard let otpCode = otpField.text, !otpCode.isEmpty else {
            showToast(message: "Please fill OTP verify", chooseImageToast: .warning)
            return
        }
